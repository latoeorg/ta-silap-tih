...
const attendance = {
  ...
  actions: {
    ...
    batchCreate: async context => {
      ...
      try {
        const attendanceData = context.state.batchForm.attendances.map(attendance => ({
        ...
        console.log(error)
        toast.error(error.response?.data?.message || "Failed to create batch attendance")
    ...
    setFormUpdate: async (context, { userId, courseId, attendance }) => {
      ...
      context.commit("SET_FORM", { key: "status", value: attendance.status || "PRESENT" })
    ...
    prepareBatchForm: async (context, { courseId }) => {
      ...
      try {
        ...
        context.commit("SET_BATCH_FORM", { key: "attendances", value: attendances })
