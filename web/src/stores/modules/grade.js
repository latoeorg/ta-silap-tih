...
const grade = {
  ...
  state: {
    ...
    examTypeOptions: [
      ...
      { title: 'UTS', value: 'MIDTERM' },
      { title: 'UAS', value: 'FINAL' },
      { title: 'Tugas', value: 'ASSIGNMENT' },
      { title: 'Proyek', value: 'PROJECT' },
    ],
  },
  mutations: {
  ...
  actions: {
    getGrades: async (context, params) => {
      ...
      try {
        ...
        console.log(error)
        toast.error(error.response?.data?.message || "Failed to fetch grade data")
      } finally {
    ...
    getGradeComponents: async context => {
      ...
      try {
        ...
        console.log(error)
        toast.error(error.response?.data?.message || "Failed to fetch grade components")
      } finally {
    ...
    getComputedComponents: async context => {
      ...
      try {
        ...
        console.log(error)
        toast.error(error.response?.data?.message || "Failed to fetch computed components")
      } finally {
        context.commit("SET_LOADING", { key: "components", value: false })
      }
    },
    ...
    fetchPrerequisites: async context => {
      ...
      try {
        const [studentsRes, coursesRes] = await Promise.all([
          axiosInstance({ method: "GET", url: `/user`, params: { page_size: 100, role: 'STUDENT' } }),
          axiosInstance({ method: "GET", url: `/course`, params: { page_size: 100 } }),
        ])
    ...
    batchCreate: async context => {
      ...
      try {
        const gradeData = context.state.batchForm.grades.map(grade => ({
          examType: context.state.batchForm.examType,
        ...
        console.log(error)
        toast.error(error.response?.data?.message || "Failed to create batch grades")
    ...
    defineComponents: async context => {
      ...
      try {
        ...
        console.log(error)
        toast.error(error.response?.data?.message || "Failed to define grade components")
    ...
    setFormUpdate: async (context, { userId, courseId, examType, grade }) => {
      ...
      context.commit("SET_FORM", { key: "totalScore", value: grade.totalScore || 0 })
    },
